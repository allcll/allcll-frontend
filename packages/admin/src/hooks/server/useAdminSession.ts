...
const postAdminSessions = async (sessions: Session) => {
  const response = await fetchOnAPI('/admin/session', { method: 'POST', body: JSON.stringify(sessions) });
...
const getAdminsessions = async (userId: string) => {
  return await fetchJsonOnAPI<Session>(`/admin/session?userId=${encodeURIComponent(userId)}`);
};

/**
 * userId의 세션 정보를 조회합니다.
