...
export function getDoughnutData(data?: WishRegister[]): DoughnutData {
  if (!data) return DefaultDoughnutData;

  const dict: Record<string, number> = {};
  data.forEach(department => {
    if (!dict[department.registerDepartment ?? '']) dict[department.registerDepartment ?? ''] = 0;
...
export function getUniversityDoughnutData(
  data?: WishRegister[],
  UniversityDict?: Record<string, string[]>,
): DoughnutData {
  if (!data || !UniversityDict) return DefaultDoughnutData;
  ...
  data.forEach(department => {
    ...
    if (!dict[university]) dict[university] = 0;
...
export function getCollegeDoughnutData(data?: WishRegister[], collegeDict?: Record<string, string[]>): DoughnutData {
  if (!data || !collegeDict) return DefaultDoughnutData;
  ...
  data.forEach(department => {
    ...
    if (!dict[college]) dict[college] = 0;
