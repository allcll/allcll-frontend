...
export function BarChart({ data, className }: BarChartProps) {
  ...
  const getColor = (i: number): string =>
    Array.isArray(dataset.backgroundColor) ? (dataset.backgroundColor[i] ?? '#60a5fa') : dataset.backgroundColor;

  return (
    <svg width={CHART_WIDTH} height={CHART_HEIGHT} className={className}>
      <Group left={margin.left} top={margin.top}>
        {data.labels.map((label, i) => {
          ...
          return (
            <Bar key={label} x={x} y={innerHeight - barH} height={barH} width={xScale.bandwidth()} fill={getColor(i)} />
          );
        })}
