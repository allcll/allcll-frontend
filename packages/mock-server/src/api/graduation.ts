...
export const handlers = [
  ...
  http.get('/api/graduation/check', async () => {
    ...
    if (state.englishCertPassedOverride === undefined) {
      return HttpResponse.json(result);
    }

    const certs = result.certifications;
    const overriddenEnglish = { ...certs.english, isPassed: state.englishCertPassedOverride };
  ...
  // 5-3. PATCH /api/graduation/check/certifications/english
  http.patch('/api/graduation/check/certifications/english', async ({ request }) => {
    await delay(600);
    const state = getGraduationState();
    if (!state.isAuthenticated) {
      return new HttpResponse(null, { status: 401 });
    ...
    const body = (await request.json()) as { isPassed?: boolean };
  ...
  http.post('/api/debug/graduation/state', async ({ request }) => {
    const body = (await request.json()) as Partial<GraduationState>;
