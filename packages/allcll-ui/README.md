# 🌀 ALLCLL UI Module - 올클 UI

AllcllUI 모듈은 `ALLCLL` 서비스의 디자인 시스템을 제공합니다. \
기본 UI 를 구성하며, 추후 디자인 변경에 대응하기 위해 설계된 프로젝트입니다.\
...
├── src/
│   ├── icons/          # 아이콘 (lucide-react 재export 및 커스텀 아이콘)
...
프로젝트에 `@allcll/allcll-ui` 패키지를 설치합니다.

```bash
pnpm install @allcll/allcll-ui
```
